...
// Forking config.
var addressFormat = "^btnd([a-zA-Z0-9]{94}|[a-zA-Z0-9]{182})$"
var divisor float64 = 1000000 
...
func init() {
	...
	if rpcPort, err = strconv.Atoi(os.Getenv("RPC_PORT")); rpcPort == 0 || err != nil {
		rpcPort = 8060
		println("Using default RPC_PORT - 8060")
	}
}
...
func getStatus(res http.ResponseWriter, req *http.Request, p httprouter.Params) {
	...
	btnd := temp["result"].(map[string]interface{})["availableBalance"].(float64) / divisor
	temp["result"].(map[string]interface{})["availableBalance"] = btnd
	btnd = temp["result"].(map[string]interface{})["lockedAmount"].(float64)
	temp["result"].(map[string]interface{})["lockedAmount"] = btnd / divisor
